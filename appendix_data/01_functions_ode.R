...
gausquad.animals<-function(m,sigma,w,h,np,na,mut.strength,points,mat,degree.animal,interaction_type,interaction,W){
  ...
  if(mat == 0){
    ...
    if(interaction_type == "trade_off"){
      ...
      for (i in 1: points){
        ...
        if(interaction == "assym"){
        f <- 10*dgamma(  ((z1[i]-z2)/W + 10),shape=5,0.5)
        }else{ f <-exp(-(z1[i]- z2)^2/w^2)  
        } #exp(-(z1[i]- z2)^2/w^2) # + 2*alpha*(sign(z1[i] - z2))*(1- exp(-(z1[i]-z2)^2/w^2)) + sign(alpha))
...
#gaussian quadrature approximation used to numerically determine the integrals.
gausquad.plants<-function(m,sigma,w,h,np,na,mut.strength,points,mat,degree.plant,interaction_type,interaction,W){
  ...
  for(r in 1:A){
    for(l in 1:P){
      ...
      temp1<-gausquad.animals(m=m.temp,sigma=sigma1,w=pars$w,h=0.25,np=Np[l],na=Na[r],
                              ...
                              interaction_type=pars$interaction_type, interaction = pars$interaction,W=pars$w1)
      aij[r,l] <-temp1$G
      bij[r,l] <-temp1$B
...
cluster_run_func<-function(params_forcing, ic_f, tmax ){
  ...
  Na<-sol %>%  filter(time >= params_forcing$duration, type !="ma",type !="mp", type =="N")
  Np<-sol %>%  filter(time >= params_forcing$duration, type !="ma",type !="mp", type =="P")
  Na<-tidyr::spread(Na, key = species, v = v)
  Np<-tidyr::spread(Np, key = species, v = v)
...
n <- sol %>% filter(time == tmax, type !="ma", type !="mp", type == "N")
p <- sol %>% filter(time == tmax, type !="ma", type !="mp", type =="P")
  
  
  ddf <-as.data.frame(cbind( rep(as.character(params_forcing$web.name),each=(A+P)), 
                             rep(seq(1,(nrow(params_forcing$matrix)+ncol(params_forcing$matrix)),1)),
                             params_forcing$forcing_strength,
                             as.numeric(c(n$v, p$v)),
                             params_forcing$mut.strength[1],
                             rr$Response_time,
                             c(rr$Degree),
                             rr$response,
                             params_forcing$handling_time,
  ...
  colnames(ddf)<-c("Web","Species",'forcing_strength',"abundance","mutualism_strength", 
                   "response_time","Degree","response_thr", "handling_time", "Network_respose_time",
